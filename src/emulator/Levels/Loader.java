...
public class Loader {
	...
	private void reset() throws IOException {
		...
		m_daI = 0;
		for (int j = 0; j < 3; j++) {
			m_haI[j] = (int) ((long) (PhysEngine.m_foraI[j] + 19660 >> 1) * (long) (PhysEngine.m_foraI[j] + 19660 >> 1) >> 16);
			m_vaI[j] = (int) ((long) (PhysEngine.m_foraI[j] - 19660 >> 1) * (long) (PhysEngine.m_foraI[j] - 19660 >> 1) >> 16);
		}
	...
	public int startXDouble() {
		return levels.startX << 1;
	}

	public int startYDouble() {
	...
	public void load(Level l1) throws InvalidTrackException {
		try {
			...
			for (int k = 0; k < j; k++) {
				...
				int k1 = -j1;
				int i2 = i1;
				int j2 = PhysEngine._doIII(k1, i2);
	...
	public int _anvI(Element n1, int j) {
		int k3 = 0;
		byte byte1 = 2;
		int l3 = n1.x >> 1;
		int i4 = n1.y >> 1;
