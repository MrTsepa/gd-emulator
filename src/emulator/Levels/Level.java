package emulator.Levels;

import emulator.GameView;
import emulator.PhysEngine;
...
public class Level {
	...
	public void _ifiIV(GameView view, int k, int i1) {
		if (i1 <= pointsCount - 1) {
			...
			if (m_rI <= 0x88000) {
				...
				if (k == i1) {
					view.drawTrackLine((m_eI << 3) >> 16, (k2 + 0x10000 << 3) >> 16, (m_bI << 3) >> 16, (l2 + 0x10000 << 3) >> 16);
					return;
				}
				view.drawTrackLine((m_eI << 3) >> 16, (k2 + 0x10000 << 3) >> 16, (points[k + 1][0] << 3) >> 16, (points[k + 1][1] + 0x10000 << 3) >> 16);
				for (int i3 = k + 1; i3 < i1; i3++)
					view.drawTrackLine((points[i3][0] << 3) >> 16, (points[i3][1] + 0x10000 << 3) >> 16, (points[i3 + 1][0] << 3) >> 16, (points[i3 + 1][1] + 0x10000 << 3) >> 16);

				view.drawTrackLine((points[i1][0] << 3) >> 16, (points[i1][1] + 0x10000 << 3) >> 16, (m_bI << 3) >> 16, (l2 + 0x10000 << 3) >> 16);
			}
		}
	}
	...
	public synchronized void _aiIV(GameView view, int k, int i1) {
		...
		int k3 = PhysEngine._doIII(i3, j3);
		...
		do {
			...
			int l3 = PhysEngine._doIII(i3, j3);
			i3 = (int) (((long) i3 << 32) / (long) (l3 >> 1 >> 1) >> 16);
			j3 = (int) (((long) j3 << 32) / (long) (l3 >> 1 >> 1) >> 16);
			view.drawTrackLine((points[j2][0] + j1 << 3) >> 16, (points[j2][1] + l1 << 3) >> 16, (points[j2 + 1][0] + i3 << 3) >> 16, (points[j2 + 1][1] + j3 << 3) >> 16);
			view.drawTrackLine((points[j2][0] << 3) >> 16, (points[j2][1] << 3) >> 16, (points[j2][0] + j1 << 3) >> 16, (points[j2][1] + l1 << 3) >> 16);
			if (j2 > 1) {
		...
		int k1 = i3;
		int i2 = j3;
		view.drawTrackLine((points[pointsCount - 1][0] << 3) >> 16, (points[pointsCount - 1][1] << 3) >> 16, (points[pointsCount - 1][0] + k1 << 3) >> 16, (points[pointsCount - 1][1] + i2 << 3) >> 16);
...
//			_ifiIV(view, k2, l2);
	...
	public synchronized void _aiV(GameView view) {
		...
		do {
			if (k >= pointsCount - 1)
				break;
			view.drawTrackLine((points[k][0] << 3) >> 16, (points[k][1] << 3) >> 16, (points[k + 1][0] << 3) >> 16, (points[k + 1][1] << 3) >> 16);
			if (m_gotoI == k) {
